...
namespace Dmontoto\GmapsBundle\API;
...
class GMap {
    ...
    public function getJavascript() {
        ...
        $init_events[] = $this->getMarkerImagesJs();
...
';
    ...
    /**
     * Adds an markerImage to be loaded
     * @param GMapMarkerImage $markerImage A google Map GMarkerImage
     */
    public function addMarkerImage($markerImage) {
        $this->markerImages[$markerImage->getName()] = $markerImage;
    }

    /**
     * returns the GMarkerImage corresponding to a name
     *
     * @param string $name
     * @return GMarkerImage
     *
     ...
     * @since 2011-09-21
     */
    public function getMarkerImageByName($name) {

        return $this->markerImages[$name];
    }
    ...
    /**
     * Returns the javascript string which defines the markerImages
     * @return string
     */
    public function getMarkerImagesJs() {
        $return = '';
        foreach ($this->markerImages as $markerImage) {
            $return .= $markerImage->ToJs();
        }
    ...
    public function getMarkersJs() {
        $return = '';
        if($this->markers > 0){
            $return .= '   var markers = [];'.PHP_EOL;
        }
        foreach ($this->markers as $marker) {
            $return .= $marker->toJs($this->getJsName()).PHP_EOL;
            $return .= 'markers.push('.$marker->getName().');'.PHP_EOL;
        }
        if($this->markers > 0){
            $return .= 'var markerCluster = new MarkerClusterer('.$this->getJsName().', markers, {maxZoom: 10});'.PHP_EOL;    
        }
